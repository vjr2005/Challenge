...
The root `Project.swift` defines all targets (app, UI tests, and all module targets) and all schemes. There are no per-module `Project.swift` files.

```swift
// Project.swift (root — all targets and schemes)
let project = Project(
    name: appName,
    options: .options(
...
```swift
// Workspace.swift — no custom schemes, only autogenerated workspace schemes
