...
Each module is an **independent Tuist project** with its own `Project.swift`. The root `Project.swift` only contains the app target and UI tests. App schemes are defined at **workspace level** in `Workspace.swift`.

```swift
// Project.swift (root — app + UI tests only)
let project = App.project
```

```swift
// {ModuleDir}/Project.swift (each module)
let project = coreModule.project  // (varies by module)
```

```swift
// Workspace.swift — includes all module projects, hosts app schemes
