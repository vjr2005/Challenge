...
struct FileManagerFileSystemTests {
	private let sut: FileSystemContract = FileManager.default
	...
	func throwsErrorWhenFileAttributesAreUnavailable() {
		...
		#expect(throws: (any Error).self) {
			// When
			try sut.fileAttributes(at: fileURL)
		}
	}
	...
	func updatesModificationDate() throws {
		// Given
		let directory = makeTemporaryDirectory()
...
private extension FileManagerFileSystemTests {
